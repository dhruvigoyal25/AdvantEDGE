...
func TestAppListGET(t *testing.T) {
	...
	applicationListAppList.VendorSpecificExt = new(ApplicationListVendorSpecificExt)
	applicationListAppList.VendorSpecificExt.VendorId = vendorId1
	...
	// Convert into JSON
	var expected_json_response = &appList
	log.Info("expected json response: ", expected_json_response)
	...
	fmt.Println("respBody: ", respBody)
	// receivedAppList = convertJsonToApplicationList(rr)
	if !validateApplicationList(&respBody, appList) {
		t.Errorf("handler returned unexpected body: got %v want %v", respBody, appList)
	}
	log.Info("Received expected response")
	// Unknown appName
	...
	if !validateApplicationList(&respBody, emptyApplicationList) {
		t.Errorf("handler returned unexpected body: got %v want %v", respBody, appList)
	}
	log.Info("Received expected response")
...
func validateApplicationList(appInfoListEntry *ApplicationList, appInfoList ApplicationList) bool {
	...
	for i, appList := range appInfoListEntry.AppList {
		if appList.VendorSpecificExt.VendorId != appInfoList.AppList[i].VendorSpecificExt.VendorId {
